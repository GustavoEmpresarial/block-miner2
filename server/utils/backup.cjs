...
function getBackupConfig() {
  ...
  let cloudCommandTemplate = String(process.env.BACKUP_CLOUD_COMMAND || "").trim();
  const cloudRemote = String(process.env.BACKUP_CLOUD_REMOTE || "gdrive").trim() || "gdrive";
  const cloudFolderId = String(process.env.BACKUP_CLOUD_FOLDER_ID || "").trim();
  if (cloudBackupEnabled && !cloudCommandTemplate && cloudFolderId) {
    cloudCommandTemplate =
      `rclone copy "{backupFile}" "${cloudRemote}:${cloudFolderId}" ` +
      "--transfers 1 --checkers 1 --drive-chunk-size 32M --fast-list";
  }
