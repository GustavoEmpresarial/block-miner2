...
export default function Inventory() {
    ...
    const groupedInventory = useMemo(() => {
        ...
        for (const item of inventory) {
            // Group by name, level, and hashRate to assure they are identical
    ...
    const onToggle = async (machineId, currentStatus) => {
        const res = await toggleMachine(machineId, !currentStatus);
        if (res.ok) {
            toast.success(res.message);
            setSelectedSlot(prev => ({ ...prev, machine: { ...prev.machine, is_active: !currentStatus } }));
        } else {
            toast.error(res.message);
        }
    };
        ...
        <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="px-4 py-2 bg-purple-500/10 border border-purple-500/20 rounded-xl text-xs font-bold text-purple-400 flex items-center gap-1.5">
                        <Zap className="w-3.5 h-3.5" />
                        {activeMachinesHashRate.toFixed(2)} GH/s
                    </div>
                            ...
                            return (
                                <div key={rackIndex} className="bg-surface border border-gray-800/50 rounded-3xl overflow-hidden shadow-xl">
                                    <div className="px-6 py-4 bg-gray-800/20 border-b border-gray-800/50 flex justify-between items-center">
                                        ...
                                        <button
                                            onClick={() => clearRack(rackIndex)}
                                            className="p-1.5 text-gray-500 hover:text-red-400 transition-colors"
                                            title={t('inventory.clear_rack')}
                                        >
                                        ...
                                        {Array.from({ length: SLOTS_PER_RACK }).map((_, localI) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`
                                                        relative aspect-square rounded-2xl border transition-all duration-300 group flex items-center justify-center
                                                        ${isDouble ? 'col-span-2 aspect-auto' : ''}
                                                        ${isOccupied
                                                            ? 'bg-gray-800/40 border-gray-700/50 hover:border-primary/40 hover:bg-primary/5 cursor-grab active:cursor-grabbing'
                                                    ...
                                                    {isOccupied ? (
                                                        <>
                                                            <img
                                                                ...
                                                                alt={descriptor.name}
                                                                className="w-4/5 h-4/5 object-contain p-2 group-hover:scale-110 transition-transform pointer-events-none"
                            ...
                            <div className="space-y-3 max-h-[60vh] overflow-y-auto scrollbar-hide pr-1">
                                {groupedInventory.map((group) => {
                                    const descriptor = getMachineDescriptor({
                                        hash_rate: group.hashRate || group.hash_rate,
                                        slot_size: group.slotSize || group.slot_size,
                                        image_url: group.imageUrl || group.image_url
                                    });
                                    ...
                                    return (
                                        <div
                                            key={group.id}
                                            draggable
                                            onDragStart={(e) => handleInventoryDragStart(e, group)}
                                            className="bg-gray-800/30 border border-gray-800/50 rounded-2xl p-4 flex items-center gap-4 hover:border-gray-700 transition-all group-item cursor-grab active:cursor-grabbing"
                                        >
                                            <div className="w-14 h-14 bg-gray-900/50 rounded-xl p-2 border border-gray-800/50 shrink-0 relative pointer-events-none">
                                                ...
                                                <div className="absolute -top-2 -right-2 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg border border-primary/20 pointer-events-auto">
                                                    x{group.quantity}
                                                </div>
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-3 mt-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                                                    ...
                                                    <span>•</span>
                                                    <span className="text-primary">{Math.round(group.hashRate || group.hash_rate)} GH/S</span>
                                                </div>
                                            </div>
                                        </div>
                                    );
                            ...
                            <div className="px-8 pt-8 pb-6 flex items-center justify-between border-b border-gray-800/50">
                                <div>
                                    <h3 className="text-xl font-bold text-white">
                                        {selectedSlot.machine ? t('inventory.modal.details_title') : t('inventory.modal.install_title')}
                                    </h3>
                                    <p className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-widest">
                                        {t('inventory.modal.rack_slot', { rack: selectedSlot.rackIndex, slot: selectedSlot.localSlotIndex + 1 })}
                                    </p>
                                </div>
                                <button onClick={() => setIsActionModalOpen(false)} className="w-10 h-10 rounded-xl bg-gray-800/50 text-gray-400 flex items-center justify-center hover:text-white transition-colors">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-4 mt-1">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-sm font-bold text-gray-300">{selectedSlot.machine.level}</span>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-bold text-gray-600 uppercase">{t('inventory.modal.hashrate')}</span>
                                                        <span className="text-sm font-bold text-primary">{Math.round(selectedSlot.machine.hashRate || selectedSlot.machine.hash_rate)} GH/s</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <button onClick={() => onRemove(selectedSlot.machine.id)} className="w-full py-4 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-2xl font-bold text-sm transition-all border border-red-500/20 flex items-center justify-center gap-2">
                                            <Trash2 className="w-4 h-4" /> {t('inventory.modal.remove_to_inventory')}
                                        </button>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        <p className="text-gray-400 text-sm font-medium mb-4">{t('inventory.modal.choose_machine')}</p>
                                        {inventory.length === 0 ? (
                                                    ...
                                                    <button key={group.id} onClick={() => onInstall(group.items[0].id)} className="w-full p-4 bg-gray-800/30 hover:bg-primary/10 border border-gray-800 hover:border-primary/30 rounded-2xl flex items-center justify-between transition-all group-item">
                                                        <div className="flex items-center gap-3 text-left">
                                                            <div className="w-10 h-10 bg-gray-900 rounded-lg p-2 shrink-0 relative">
                                                                <img src={getMachineDescriptor({
                                                                    hash_rate: group.hashRate || group.hash_rate,
                                                                    slot_size: group.slotSize || group.slot_size,
                                                                    image_url: group.imageUrl || group.image_url
                                                                }).image} className="w-full h-full object-contain" />
                                                                <div className="absolute -top-2 -right-2 bg-primary text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-lg border border-primary/20">
                                                                    x{group.quantity}
                                                                </div>
                                                            </div>
                                                            <div>
                                                                ...
                                                                <div className="text-[10px] font-bold text-gray-500 uppercase">{Math.round(group.hashRate || group.hash_rate)} GH/s</div>
                                                            </div>
                                                        </div>
